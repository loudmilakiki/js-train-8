...
function getUsersFromDatabase(array) {
  const user = Array.from(array, (item) => ({
    id: item.id,
...
function filterByCondition(arr, condition) {
  const filterCondition = arr.filter(condition);
...
function checkArray(obj) {
  if (obj === null || obj === undefined) {
    obj = "Помилка: Вхідний об'єкт є null або undefined";
    return false;
  } else {
...
function customUnshift(arr, ...elements) {
  ...
  for (let item = elements.length - 1; item >= 0; item--) {
    arr.unshift(elements[item]);
  }
...
function customIncludes(arr, element) {
  ...
  const includElem = arr.includes(element);
  const item = arr.filter((el) => el === element).length;
  console.log(item);
  return includElem;
  // Перевіряємо, чи вхідний параметр є масивом
